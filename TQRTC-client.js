const TQRTC = function() {
    const PeerConnection = (window.PeerConnection || window.webkitPeerConnection00 || window.webkitRTCPeerConnection || window.mozRTCPeerConnection);
    const URL = (window.URL || window.webkitURL || window.msURL || window.oURL);
    ...
    tqrtc.prototype = new EventEmitter();
    ...
    //本地连接信道，信道为websocket
    tqrtc.prototype.connect = function(server, room) {
        var socket,
            that = this;
        room = room || "";
    ...
    //创建本地流
    tqrtc.prototype.createStream = function(options) {
        var that = this;
    ...
    tqrtc.prototype.addStreams = function() {
        var i, m,
            stream,
            connection;
    ...
    //将流绑定到video标签上用于输出
    tqrtc.prototype.attachStream = function(stream, domId) {
    ...
    tqrtc.prototype.sendOffers = function() {
        var i, m,
            pc,
            that = this,
    ...
    tqrtc.prototype.receiveOffer = function(socketId, sdp) {
        var pc = this.peerConnections[socketId];
        this.sendAnswer(socketId, sdp);
    };

    //发送answer类型信令
    tqrtc.prototype.sendAnswer = function(socketId, sdp) {
        var pc = this.peerConnections[socketId];
        var that = this;
    ...
    tqrtc.prototype.receiveAnswer = function(socketId, sdp) {
        var pc = this.peerConnections[socketId];
        pc.setRemoteDescription(new nativeRTCSessionDescription(sdp));
    };
    ...
    tqrtc.prototype.createPeerConnections = function() {
        var i, m;
        for (i = 0, m = this.connections.length; i < m; i++) {
    ...
    //创建单个PeerConnection
    tqrtc.prototype.createPeerConnection = function(socketId) {
        var that = this;
    ...
    //关闭PeerConnection连接
    tqrtc.prototype.closePeerConnection = function(pc) {
        if (!pc) return;
        pc.close();
    };
    ...
    //消息广播
    tqrtc.prototype.broadcast = function(message) {
        var socketId;
    ...
    //发送消息方法
    tqrtc.prototype.sendMessage = function(message, socketId) {
    ...
    tqrtc.prototype.addDataChannels = function() {
        var connection;
        for (connection in this.peerConnections) {
    ...
    tqrtc.prototype.createDataChannel = function(socketId, label) {
        var pc, key, channel;
    ...
    tqrtc.prototype.addDataChannel = function(socketId, channel) {
        var that = this;
    ...
    tqrtc.prototype.parseFilePacket = function(json, socketId) {
    ...
    tqrtc.prototype.shareFile = function(dom) {
        var socketId,
            that = this;
    ...
    //向某一单个用户发送文件
    tqrtc.prototype.sendFile = function(dom, socketId) {
        var that = this,
            file,
            reader,
            fileToSend,
    ...
    //发送多个文件的碎片
    tqrtc.prototype.sendFileChunks = function() {
        var socketId,
            sendId,
            that = this,
            nextTick = false;
    ...
    //发送某个文件的碎片
    tqrtc.prototype.sendFileChunk = function(socketId, sendId) {
        var that = this,
    ...
    //发送文件请求后若对方同意接受,开始传输
    tqrtc.prototype.receiveFileAccept = function(sendId, socketId) {
        var that = this,
            fileToSend,
            reader,
    ...
    tqrtc.prototype.receiveFileRefuse = function(sendId, socketId) {
        var that = this;
    ...
    //清除发送文件缓存
    tqrtc.prototype.cleanSendFile = function(sendId, socketId) {
        var that = this;
        delete that.fileChannels[socketId][sendId];
    };

    //发送文件请求
    tqrtc.prototype.sendAsk = function(socketId, sendId, fileToSend) {
        var that = this,
            channel = that.dataChannels[socketId],
            packet;
        if (!channel) {
    ...
    //获得随机字符串来生成文件发送ID
    tqrtc.prototype.getRandomString = function() {
    ...
    //接收到文件碎片
    tqrtc.prototype.receiveFileChunk = function(data, sendId, socketId, last, percent) {
        var that = this,
    ...
    tqrtc.prototype.getTransferedFile = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
    ...
    //接收到发送文件请求后记录文件信息
    tqrtc.prototype.receiveFileAsk = function(sendId, fileName, fileSize, socketId) {
        var that = this;
    ...
    //发送同意接收文件信令
    tqrtc.prototype.sendFileAccept = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
    ...
    //发送拒绝接受文件信令
    tqrtc.prototype.sendFileRefuse = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],
            channel = that.dataChannels[fileInfo.socketId],
            packet;
    ...
    //清除接受文件缓存
    tqrtc.prototype.cleanReceiveFile = function(sendId) {
        var that = this;
